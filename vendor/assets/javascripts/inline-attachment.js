 ...
 * Minor local modifications: biomancer
 */
(function(document, window) {
  ...
  // new function for quotation with images
  inlineAttachment.prototype.uploadFileForQuote = function(file) {
    var me = this,
      formData = new FormData(),
      xhr = new XMLHttpRequest(),
      settings = this.settings,
      ...
      var string = '<blockquote>Текст цитаты\n    <div class="author">\n        <div class="author-photo" style="background-image: url(' + "'{filename}'" + ')"></div>\n        <div class="author-info">Имя и фамилия автра цитаты <span>Рабочая должность</span></div>\n    </div>\n</blockquote>';

      settings.urlText = string;
      settings.extraParams.variation = 'quote_photo'
  ...
  inlineAttachment.prototype.onFileSelect = function(files) {
    var result = false;
  ...
  inlineAttachment.prototype.onFileSelectForQuote = function(files) {
    var result = false;
    for (var i = 0; i < files.length; i++) {
      var file = files[i];
      if (this.isFileAllowed(file)) {
